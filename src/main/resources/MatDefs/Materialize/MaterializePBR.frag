...
// Materialize
    ...
    uniform float m_EffectTime;
    uniform vec4 m_EdgeColor;
    uniform float m_EdgeThickness;
#endif
...
void main(){
    ...
    #ifdef MATERIALIZING
        float tex_scale = 0.5;
        vec4 noise = texture2D(m_Noise, texCoord * tex_scale);
        alpha = step(1.0 - m_EffectTime, noise.r);
    #endif
    // END Materialize
    ...
    #ifdef MATERIALIZING
        float border = step((1.0 - m_EffectTime), noise.r - m_EdgeThickness);
        vec4 emissive = mix(m_EdgeColor, vec4(0.0, 0.0, 0.0, 0.0), border);

        float emPower = 3.0;
        float emIntentisy = 2.0;

        gl_FragColor += emissive * pow(emissive.a, emPower) * emIntentisy;
    #endif
